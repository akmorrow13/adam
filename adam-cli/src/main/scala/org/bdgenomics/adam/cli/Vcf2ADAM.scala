...
class Vcf2ADAM(val args: Vcf2ADAMArgs) extends BDGSparkCommand[Vcf2ADAMArgs] with DictionaryCommand with Logging {
  ...
  def run(sc: SparkContext) {
    ...
    val adamVariants: VariationRDD[VariantContext] = sc.loadVcf(args.vcfPath, sd = dictionary)
    val coalesceRDD: RDD[VariantContext] =
      if (args.coalesce > 0) {
        if (args.coalesce > adamVariants.rdd.partitions.size || args.forceShuffle) {
          adamVariants.rdd.coalesce(args.coalesce, shuffle = true)
        } else {
          adamVariants.rdd.coalesce(args.coalesce, shuffle = false)
        }
      } else adamVariants.rdd

    coalesceRDD.saveAsParquet(args, args.onlyVariants, adamVariants.sequences)
